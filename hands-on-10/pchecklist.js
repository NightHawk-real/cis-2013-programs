...
var passwordCheckOld = function () 
{
    ...
    {
        $("password").value = "";  // resets the input field to blank
        $("output").value = "Password too short. Your passwords should always be at least more than 8 characters long. Please try again."; // outputs error msg to textarea 
    }
    else
    {
        for(x = 0; x < arrayBadPasswords.length; x++)
        {
            if(stringPassword === arrayBadPasswords[x])  // as long as the counter, x, is < the length of the arrayBadPassWords, the loop will continue checking if the input password matches any of the 500 bad passwords.
            {
                $("output").value = "Your password: \"" + stringPassword + "\"" +" matches one of the 500 bad passwords. Please try a new, more unique password."; // outputs corresponding non-unique password error message
                break;
            }
            else
            {
                $("output").value = "Your password is strong! However, passwords can always be stronger. Try using a password manager to remember even stronger, more complicated passwords."; // outputs confirmation message that password is strong.
            }
        }
    }
};
